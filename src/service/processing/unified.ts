...
import { buildInstructionPrompt, getOutputFormatInstruction, getIntermediateRecognitionInstruction } from "../instructions/utils";
import { DEFAULT_API_URL, DEFAULT_MODEL, getGPTInstance, isImageData, unwrapUnwantedCodeBlocks, getResponseFormat } from "../shared/gpt-utils";
import { loadAISettings, getActiveCustomInstruction, getLanguageInstruction, getSvgGraphicsAddon } from "./settings";
import { RecognitionCache } from "../recognition/cache";
import { detectPlatform, getPlatformAdapter } from "./adapters";
import { solveAndAnswer, writeCode, extractCSS } from "./core";
import { extractEntities } from "./entities";
import { smartRecognize } from "../recognition/smart";
import type {
	AIConfig,
...
export const processDataWithInstruction = async (
	...
	if (signal?.aborted) {
		const result: ProcessDataWithInstructionResult = { ok: false, error: "Cancelled" };
		sendResponse?.(result);
		return result;
	}
