import { $t as navigate, Dn as orientationNumberMap, En as getCorrectOrientation, H as listVirtualRootEntriesFromRouter, Ht as elementPointerMap, Jt as H, On as updateVP, Qt as registerModal, Sn as preloadStyle, U as resolveFsBackend, Yt as vector2Ref, an as numberRef, bn as loadAsAdopted, it as getWallpaperStoragePointer, jn as isInFocus, kn as MOCElement, kt as pointerAnchorRef, nn as affected, on as observe, p as openUnifiedContextMenu, s as installLauncherBackStack, sn as propRef, st as setAppWallpaperFromBlob, t as home_host_apply_default, tn as M, vn as __vitePreload, wn as ensureVirtualKeyboardOverlay, yn as DOMMixin, yt as handleIncomingEntries } from "../com/app.js";
...
async function launchEcosystemSku(sku) {
	const { androidPackageForSku, isCwspSku } = await __vitePreload(async () => {
		const { androidPackageForSku, isCwspSku } = await import("./ecosystem-skus2.js");
		return {
...
async function tryLaunchSiblingView(view) {
	try {
		const { readCwspSku, siblingSkuForView } = await __vitePreload(async () => {
			const { readCwspSku, siblingSkuForView } = await import("./ecosystem-skus2.js");
			return {
				readCwspSku,
				siblingSkuForView
			};
