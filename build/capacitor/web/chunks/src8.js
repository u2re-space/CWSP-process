...
import { Mn as H, Yt as StorageKeys, Zt as setString, er as observe, lr as removeAdopted, nr as ref, ur as __vitePreload } from "../com/app.js";
import { o as SettingsChannelAction } from "../views/viewer.js";
import "./storage.js";
import { A as canonicalHubSettingsSection, B as visibleHubSettingsSections, Ct as saveSettings, F as readSettingsAreaSection, Fn as normalizeEcosystemToken, I as rememberSettingsAreaSection, In as resolveEcosystemToken, L as resolveEffectiveHubSettingsSection, M as hasBuiltInSettingsPanel, N as hubSettingsSectionPath, Nn as BUILTIN_AI_MODELS, P as pruneBuiltInSettingsTabs, R as resolveSettingsShellProfile, St as noteSettingsControlSync, Wn as sendMessage, Z as DEFAULT_INSTRUCTION_TEMPLATES, ar as mergeOpenPolicy, at as isEnabledView, bt as getLastSettingsSaveReport, ct as applyTheme, dr as resolveHostOpenPolicy, dt as FALLBACK_BASE_COLOR, ft as defaultColorSource, j as defaultSettingsTabForProfile, k as SIBLING_HUB_SETTINGS_SECTIONS, kn as resolveCwspUrlFields, mt as normalizeHexColor, pt as isAppearanceColorSource, t as navigateToView, vr as stampHostOpenPolicy, vt as ensureCapacitorCwspSettingsSeeded, wt as applyAirpadRuntimeFromAppSettings, xt as loadSettings, yt as ensureCrxCwspSettingsSeeded, z as skuForHubSettingsSection } from "../shells/boot-index.js";
...
var resolveCwspSettingsBeforeSave = async (settings) => {
	...
	const { sanitizeFleetSelfWireNodeId } = await __vitePreload(async () => {
		const { sanitizeFleetSelfWireNodeId } = await import("../shells/boot-index.js").then((n) => n.on);
...
var createSettingsView = (opts) => {
	...
	Promise.resolve(loadSettingsForView()).then((s) => {
		...
		if (isCapacitorNative()) __vitePreload(() => import("../shells/boot-index.js").then((n) => n.Zt).then(async (m) => {
...
var SettingsView = class {
	...
	invokeChannelApi(action, payload) {
		if (action === SettingsChannelAction.Patch || action === SettingsChannelAction.SettingsUpdate) {
			...
			(async () => {
				try {
					const [{ loadSettings }, { applyTheme }] = await Promise.all([__vitePreload(() => import("../shells/boot-index.js").then((n) => n._t), __vite__mapDeps([4,2,5,1,6,7,8]), import.meta.url), __vitePreload(() => import("../shells/boot-index.js").then((n) => n.st), __vite__mapDeps([4,2,5,1,6,7,8]), import.meta.url)]);
					const cur = await loadSettings();
					const patch = payload;
					applyTheme({
						...cur,
