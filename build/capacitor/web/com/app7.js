...
import { t as processDataWithInstruction } from "../chunks/unified.js";
...
import { t as WorkCenterStateManager } from "../chunks/WorkCenterState.js";
...
var WorkCenterShareTarget = class {
	...
	async addShareTargetResult(state, resultData) {
		...
		const { WorkCenterStateManager } = await __vitePreload(async () => {
			const { WorkCenterStateManager } = await import("../chunks/WorkCenterState.js").then((n) => n.n);
	...
	async addShareTargetInput(state, inputData) {
		...
		try {
			...
			const { WorkCenterStateManager: StateManager } = await __vitePreload(async () => {
				const { WorkCenterStateManager: StateManager } = await import("../chunks/WorkCenterState.js").then((n) => n.n);
...
var tryProcessApiTurn = async (input, options) => {
	...
	if (!text) return null;
	const auth = processApiAuthFromSettings(await loadSettings().catch(() => null));
...
var WorkCenterActions = class {
	...
	async viewResultsInViewer(state) {
		...
		try {
			...
			}, __vite__mapDeps([4,1,5,6,7,8,9,10]), import.meta.url);
...
var WorkCenterDataProcessing = class {
	...
	renderAsMarkdown(data) {
		...
		if (!renderedContent.trim()) try {
			return renderSafeMarkdown(this.extractTextContent(data));
		} catch (error) {
...
var extractPdf = async (file) => {
	const [{ getDocument, GlobalWorkerOptions }, worker] = await Promise.all([__vitePreload(() => import("../vendor/pdfjs-dist.js"), __vite__mapDeps([11,6,7,1]), import.meta.url), __vitePreload(() => import("../vendor/pdfjs-dist2.js"), [], import.meta.url)]);
...
var extractDocx = async (file) => {
	const mammothModule = await __vitePreload(() => import("../vendor/underscore.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)), __vite__mapDeps([12,1]), import.meta.url);
...
var extractXlsx = async (file) => {
	const xlsxModule = await __vitePreload(() => import("../vendor/xlsx.js"), [], import.meta.url);
...
var WorkCenterManager = class {
	...
	async handleIncomingContent(data, contentType) {
		...
		try {
			...
			if (text.trim() && attached.length === 0 && !isAndroidLocalShareUri(text)) if (shareLike) {
