...
import { Br as persistSpeedDialMeta, Cr as peekOpenPolicy, Dr as resolveOpenPlacement, Er as resolveHostOpenPolicy, Fr as viewIdForOpenSink, Ir as addSpeedDialItem, Lr as createEmptySpeedDialItem, Mr as skuForOpenSink, Rr as ensureSpeedDialMeta, Tr as resolveExplorerOpenSink, Vr as speedDialItems, bt as loadSettings, gr as classifyOpenKindFromName, hr as classifyOpenKind, jr as sinkToOpenLinkTarget, wr as rememberOpenPolicyFromSettings, yr as looksLikePreviewableBinary, zr as persistSpeedDialItems } from "../shells/boot-index.js";
...
function wireExplorerSubtree(shellRoot, wireOpts) {
	...
	if (fm) {
		...
		__vitePreload(() => import("../com/app.js").then((n) => n.d).then((m) => m.installExplorerBackStack()), __vite__mapDeps([3,2]), import.meta.url).catch(() => {});
		return {
			cleanup: () => {
