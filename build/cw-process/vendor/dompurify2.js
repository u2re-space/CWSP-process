	...
	return class ViewerView extends Base {
		...
		*/
		...
		async goPathFromBar() {
			...
			if (await this.openMarkdownFromUrl(src)) return;
			if (native) {
				try {
					const { getAllFilesStatus, isNativeStorageAvailable } = await __vitePreload(async () => {
						const { getAllFilesStatus, isNativeStorageAvailable } = await import("../com/app.js").then((n) => n.m);
						return {
							getAllFilesStatus,
					...
					if (isNativeStorageAvailable()) {
						if (!(await getAllFilesStatus()).allFilesAccess) {
			...
			this.showMessage("Could not open that path or URL");
		}
